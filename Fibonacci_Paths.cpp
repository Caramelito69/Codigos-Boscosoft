...
#define printv(v) for(auto e:vec)cout<<e<<' ';cout<<endl;
#define F first
#define S second
#define MP make_pair
...
int solve(int idx)
{
    if(me[idx]!=-1)return me[idx];
    int u=bus[idx].F,v=bus[idx].S,target=a[u]+a[v],res=1;
    ii p={target,-1};
    auto it=lower_bound(all(adj[v]),p);
    while(it!=adj[v].end()&&it->F==target)
        res=(res+solve(it->S))%MOD,it++;
    return me[idx]=res;
}
void solve()
{
