...
void solve()
{
    ...
    fore(i,2,n-2)
    {
        while (k+1< i)
        {
            ll dif1=abs(p[k]-(p[i]-p[k]));
            ll dif2=abs(p[k+1]-(p[i]-p[k+1]));
            if (dif2<=dif1)
                k++;
            else
                break;
        ...
        while (m+1<n)
            {
            ll s3_actual=p[m]-p[i];
            ll s4_actual=p[n]-p[m];
            ll s3_sig=p[m+1]-p[i];
            ll s4_sig=p[n]-p[m+1];
            if (abs(s3_sig-s4_sig)<=abs(s3_actual-s4_actual))
                m++;
            else
                break;
        }
        ll s1=p[k],s2=p[i]-p[k],s3=p[m]-p[i],s4=p[n]-p[m];
        ll max_s=max({s1, s2, s3, s4}),min_s=min({s1, s2, s3, s4});
        ll dif=max_s-min_s;
        if (men==-1||dif<men)
            men=dif;
    }
