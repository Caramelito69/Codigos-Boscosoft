...
#define readi2(a,b) int n;cin>>a>>b;
#define readi3(a,b,c) int n;cin>>a>>b>>c;
#define readi4(a,b,c,d) int n;cin>>a>>b>>c>>d;
...
#endif
ll randint(ll ini,ll fin)
{
    return uniform_int_distribution<int>(ini,fin)(mt_rng);
}
const int ID_X=0;
const int ID_Y=1;
const int ID_Z=2;
...
{
    ll x,y,z,e;
    if(!(cin>>x>>y>>z>>e))return;
    vi vec(e);
    for(auto &e:vec)cin>>e;
    ll arr[3]={x,y,z},b[3]={0,0,0},c=0;
    fore(i,0,e-1)
        {
        ll r=vec[i];
        vi val;
        if(arr[ID_X]>=r) val.PB(ID_X);
        if(arr[ID_Y]>=r) val.PB(ID_Y);
        if(arr[ID_Z]>=r) val.PB(ID_Z);
        if(val.empty())
            {cout<<-1<<"\n";return;}
        ll men=inf;
        for(int id:val)
