...
struct ST
{
    ...
    ST(int sz)
    {
        ...
        vec3.assign(4*n+5,false);
    }
    void build(int nodo,int ini,int fin,vi &arr)
    {
        if (ini==fin)
        {
            tree[nodo]=arr[ini];
        ...
        int mid=(ini+fin)/2;
        build(2*nodo,ini,mid,arr);
        build(2*nodo+1,mid+1,fin,arr);
    ...
    void push(int nodo, int ini, int fin)
    {
        ...
        if (vec3[nodo])
        {
            vec3[izq]=true,lzSet[izq]=lzSet[nodo],lzAdd[izq]=0;
            tree[izq]=lzSet[nodo]*(mid-ini+1);
            vec3[der]=true,lzSet[der]=lzSet[nodo],lzAdd[der]=0;
            tree[der]=lzSet[nodo]*(fin-mid);
            vec3[nodo]=false,lzSet[nodo]=0;
        }
        if (lzAdd[nodo]>0)
        {
            if (vec3[izq]) lzSet[izq]+=lzAdd[nodo];
            else lzAdd[izq]+=lzAdd[nodo];
            tree[izq]+=lzAdd[nodo]*(mid-ini+1);

            if (vec3[der]) lzSet[der]+=lzAdd[nodo];
            else lzAdd[der]+=lzAdd[nodo];
            tree[der]+=lzAdd[nodo]*(fin-mid);
            lzAdd[nodo]=0;
        }
    }
    void update(int nodo, int ini, int fin, int l, int r, int val, int t)
    {
        if (r<ini || fin<l) return;
        if (l<=ini && fin<=r)
        {
            if (t==2)
            {
                vec3[nodo]=true,lzSet[nodo]=val,lzAdd[nodo]=0;
                tree[nodo]=val*(fin-ini+1);
            }
            else
            {
                if (vec3[nodo])lzSet[nodo]+=val;
                else lzAdd[nodo]+=val;
                tree[nodo]+=val*(fin-ini+1);
            }
            return;
    ...
    {
        if (r<ini || fin<l) return 0;
        if (l<=ini && fin<=r) return tree[nodo];
        push(nodo,ini,fin);
        int mid=(ini+fin)/2;
